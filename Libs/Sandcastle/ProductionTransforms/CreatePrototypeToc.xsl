...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:param name="rootNamespaceContainer" select="'false'" />
	...
	<xsl:template match="/">
		<topics>
			<xsl:choose>
				<xsl:when test="$rootNamespaceContainer = 'true' and count(/reflection/apis/api[topicdata/@group='root']) > 0">
	...
	<xsl:template match="api[topicdata/@group='root']">
