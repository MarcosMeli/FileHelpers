...
namespace FileHelpers
{
	...
	public class RecordInfo
	{
		...
		private void InitFields()
		{
			...
			if (mRecordType.IsValueType == true)
				throw new BadUsageException("Structures are not supported in the FileHelperEngine only classes are allowed.");

			TypedRecordAttribute recordAttribute = null;

			if (mRecordType.IsDefined(typeof (TypedRecordAttribute), true) == false)
				throw new BadUsageException("The record class must be marked with the DelimitedRecord or FixedLengthRecord Attribute");
			else
			{
